...
public class ThreePrisonersDilemmaWithStrategies {
    ...
    class FirmButFairPlayer extends Player {
        ...
        // Flag to indicate the first move
        ...
        int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
            ...
            // Try to cooperate again after both opponents defect in the previous round
            else if (!cooperate) {
                cooperate = oppHistory1[n - 1] == 0 && oppHistory2[n - 1] == 0;
                // Return 0 if cooperate, 1 if defect
                return cooperate ? 0 : 1;
            }
            // Continue to cooperate until either opponent defects
            else {
                cooperate = oppHistory1[n - 1] == 0 && oppHistory2[n - 1] == 0;
                return cooperate ? 0 : 1;
            }
    ...
    class GradualPlayer extends Player {
        ...
        int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
            ...
            // If the opponent has defected at least once, start applying the gradual
            // strategy
